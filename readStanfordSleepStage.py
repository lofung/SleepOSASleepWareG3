...
with open(filepath) as fp:
    ...
    for element in splitted:
        XMLlist.append("<Event Family=\"Respiratory\" Type=\""+ str(element[3]) + "\" Start=\""+ str(element[0]) +"\" Duration=\""+ str(element[2])+"\" Machine=\"true\">")
        XMLlist.append("   <HRBefore>0</HRBefore>")
        XMLlist.append("   <HRExtreme>0</HRExtreme>")
        XMLlist.append("   <O2Before>0</O2Before>")
        XMLlist.append("   <O2Min>0</O2Min>")
        XMLlist.append("</Event>")
    #print(XMLlist)

    # save at output
    with open(folderpath + "outputRespEvents.txt", 'w') as f:
        for item in XMLlist:
            f.write("%s\n" % item)
